...
class HBController(Node):
    def __init__(self):
        ...
        self.next_index_timer = self.create_timer(0.3, self.next_index_callback)
    ...
    def normalize(self, vl, vr, vb):

        if abs(vl) > self.vel_up_lim or abs(vr) > self.vel_up_lim or abs(vb) > self.vel_up_lim  :
            norm = np.linalg.norm([vl,vr,vb])
            vl *= self.vel_up_lim/norm
            vr *= self.vel_up_lim/norm
            vb *= self.vel_up_lim/norm

            print(f"MOTOR SPEEDS: Left: {vl:.3f}, Right: {vr:.3f}, Rear: {vb:.3f}\n")
            
        else:
            print(f"MOTOR SPEEDS: Left: {vl:.3f}, Right: {vr:.3f}, Rear: {vb:.3f}\n")

        if abs(vl) < self.vel_down_lim and abs(vr) < self.vel_down_lim and abs(vb) < self.vel_down_lim  :
            norm = np.linalg.norm([vl,vr,vb])
            vl *= self.vel_down_lim/norm
            vr *= self.vel_down_lim/norm
            vb *= self.vel_down_lim/norm
    ...
    def get_pose_callback(self, msg: Pose2D):
        ...
        reduction_factor = 0.9     # good enough at 0.96 for bot height 13.3 cm
...
def main(args=None):
    ...
    while rclpy.ok():
        try:
            ...
            x_goal      = hb_controller.bot_2_x[hb_controller.index] - 250 
            # x_goal      = 50
            y_goal      = 250 - hb_controller.bot_2_y[hb_controller.index]
            # y_goal      = 50
            ...
            vl, vr, vb = hb_controller.normalize(*hb_controller.inverse_kinematics(vel_x, vel_y, vel_theta))
