...
void normalize(){
  if (abs(motors.left) > vel_threshold || abs(motors.right) > vel_threshold || abs(motors.rear) > vel_threshold){
    double norm = pow(pow(motors.left, 2.0) + pow(motors.right, 2.0) + pow(motors.rear, 2.0), 0.5);
...
//   delay(2000);
   ...
   RCCHECK(rclc_node_init_default(&node, "hb_bot_2", "", &support));
