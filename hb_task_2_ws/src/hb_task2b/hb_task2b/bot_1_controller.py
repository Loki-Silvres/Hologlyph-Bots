...
class HBController(Node):
    def __init__(self):
        ...
        self.get_logger().info("hb_controller_1 has been started.")
    ...
    def inverse_kinematics(self, vx, vy, vtheta):
        ...
        ############################################
        matrix = np.array([[-1.0, -1.732/3, 0.68],
                           [-1.0,  1.732/3, 0.68],
                           [ 2.0,      0.0, 0.68]])
    ...
    def next_index_callback(self):
        if self.goal_flag == True:
            self.index+=1
            if self.index >= len(self.bot_1_x):
                self.index = 0
...
def main(args=None):
    ...
    while rclpy.ok():
        try:
            ...
            x_goal      = hb_controller.bot_1_x[hb_controller.index] - 250 
            # x_goal      = 50
            y_goal      = 250 - hb_controller.bot_1_y[hb_controller.index]
            # y_goal      = 50
